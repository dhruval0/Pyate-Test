    ...
    if doc.is_extractable:
        ...
        try:
            if ALGO == "original":
                ...
                words1= title.split() 
                #print('LEn of Word1',len(words1))
                #print(len(words1))
                #print(title)       
                if len(title) == 0 or len(words1) <=max_words:
                    #print("I am in T0")
                    selected_blocks = []
                    for tfs_index in [1]:
                        selected_blocks.extend(
                    ...
                    title = ' '.join([str(elem) for elem in words1])
                    #print(words1)
                    #print(len(words1))          
                    if len(title) == 0 or len(words1) <=max_words:
                        #print("I am in T1")
                        selected_blocks = []
                        for tfs_index in [2]:
                            selected_blocks.extend(list(filter(lambda b: b[1] == all_tfs[tfs_index],dev.blocks)))
                        # sort the selected blocks, put y min first, then x min if y min is
                        # same
                        # 1000000 is a magic number here, assuming no x value is greater
                        # than that
            ...
            if "  " in title:
                title = " ".join(title.split())
            words= title.split()
            title = ' '.join([str(elem) for elem in words[0:max_title]])
            return title
        except IndexError:
            return title
    else:
